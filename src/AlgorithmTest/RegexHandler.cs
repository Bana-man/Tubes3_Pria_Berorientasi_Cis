...
namespace StringHandler
{
    ...
    class BoyerMoore
    {
        ...
        //static void Main()
        //{
        //    Console.WriteLine(BMMatch(Regex.strToList("ascnwivc5jav14hbdf"), Regex.strToRgx("csjaeviah")));
        //    Console.ReadLine();
        //}
    }
    class kmp
    {
        public static int kmpmatch(string text, string pattern)
        {
            ...
            string[] regex = Regex.strToRgx(pattern);
            string[] textarr = Regex.strToList(text);

            while (i < text.Length)
            {
                if (Regex.isEqualRgx(regex[j], textarr[i]) >= 0)   // char equal
                {
                ...
                else if (Regex.isOptional(regex[j]))
                {
                    j++;
                }
                else if (j > 0)
